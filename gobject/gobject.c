...
#define WEAK_REF_SYNC_DATA_LOCK_MASK (1 << WEAK_REF_SYNC_DATA_LOCK_BIT)
...
typedef struct
{
  ...
  union
  {
     ...
     * "sync_data".
     *
     * This is to safe a separate pointer for "destroy" in the long-lived
 ...
 * destrotyed. If the object was not resurrected during dispose(), any
          ...
          if (G_UNLIKELY (tuple->is_notifying))
            {
              ...
              if (!(g_atomic_int_get (&tuple->u.sync_data->w_sync_flag) & WEAK_REF_SYNC_DATA_LOCK_MASK))
                {
              ...
              if (tuple->u.sync_data->w_notifying_thread == g_thread_self ())
                {
 ...
 * wait for a concurrent @notify to completed. This means, that when the function
 ...
 * because you will need a strong reference before calling g_object_weak_unref_full().
 * But it does not cover all races, because g_object_run_dispose() also invokes
 * weak notifications, but is unaffected by #GWeakRef. In general, to properly
 * handle thread-safety you must look at the return value of this function (to
 * cleanup resources once) or at least use a @destroy notify (which also ensures
 * that the resources are released exactly once).
 *
 * Since: 2.86
 *
...
typedef struct
{
  ...
  GThread *notifying_thread_cache;
  ...
  if (tuple->synchronize)
    {
      ...
      if (!wdata->notifying_thread_cache)
        wdata->notifying_thread_cache = g_thread_self ();
      ...
      *sync_data = (WeakRefSyncData){
        .w_sync_flag = 0,
        .w_refcount = 1,
        .w_notifying_thread = wdata->notifying_thread_cache,
        .destroy = tuple->u.destroy,
      };

      /* Instead of
       *
       *   g_bit_lock (&tuple->synced.s_data->w_sync_flag, WEAK_REF_SYNC_DATA_LOCK_BIT);
       *
       * just set the locked bits here directly. We didn't publish the
       * data yet (so no other threads can access it yet). In that case,
       * g_bit_lock() would also just set these lock bits.
       */
      sync_data->w_sync_flag = (1u << WEAK_REF_SYNC_DATA_LOCK_BIT);
      ...
      if (destroy)
        {
           ...
           * callback to do anything (without worrying about dead-lock).
           *
