...
test_clear_signal_handler (void)
{
  ...
  handler1 = g_signal_connect (test_obj, "simple", G_CALLBACK (dont_reach), NULL);
  g_assert_cmpuint (handler1, >, 0);

  handler2 = g_signal_connect (test_obj, "simple", G_CALLBACK (dont_reach), NULL);
  g_assert_cmpuint (handler2, >, 0);

  g_clear_signal_handler (&handler1, test_obj);
  g_assert_cmpuint (handler1, ==, 0);

  g_clear_signal_handler (&handler2, test_obj);
  g_assert_cmpuint (handler2, ==, 0);
  ...
  if (g_test_undefined ())
    {
      handler1 = g_random_int_range (0x01, 0xFF);
