  ...
  for (i = 0; i < G_N_ELEMENTS (vectors); i++)
    {
      ...
      g_value_set_variant (&v1, (vectors[i].v1 != NULL) ?
                           g_variant_new_parsed (vectors[i].v1) : NULL);

      g_value_init (&v2, G_TYPE_VARIANT);
      g_value_set_variant (&v2, (vectors[i].v2 != NULL) ?
...
main (int argc, char *argv[])
{
  ...
  g_test_add_func ("/param/char", test_param_spec_char);
  g_test_add_func ("/param/string", test_param_spec_string);
  g_test_add_func ("/param/override", test_param_spec_override);
  g_test_add_func ("/param/gtype", test_param_spec_gtype);
  g_test_add_func ("/param/variant", test_param_spec_variant);
  g_test_add_func ("/param/variant/cmp", test_param_spec_variant_cmp);
