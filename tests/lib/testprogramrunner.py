...
class TestProgramRunner(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        program_name = f"{cls.PROGRAM_NAME}{ext}"

        if "_G_TEST_PROGRAM_RUNNER_PATH" in os.environ:
            cls.__program = os.path.join(
                os.environ["_G_TEST_PROGRAM_RUNNER_PATH"], program_name
            )
        elif "G_TEST_BUILDDIR" in os.environ:
            cls.__program = os.path.join(os.environ["G_TEST_BUILDDIR"], program_name)
        else:
            cls.__program = os.path.join(os.path.dirname(__file__), program_name)
            if not os.path.exists(cls.__program):
                cls.__program = shutil.which(program_name)
