...
def main():
    ...
    log_process = subprocess.run([
        'git', 'log', '-U0', '--no-color', args.commit_range],
    ...
    diff_process = subprocess.run([
        'git', 'diff', '-U0', '--no-color', args.commit_range],
        stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding='utf-8',
        check=True)
    ...
    if banned_keywords_seen:
        print('Saw banned keywords in a commit diff or log message: {}. '
              'This indicates the branch is a work in progress and should not '
              'be merged in its current '
              'form.'.format(', '.join(banned_keywords_seen)))
        sys.exit(1)
