...
{
  g_atomic_int_inc (&subscriber->ref_count);
  return subscriber;
}
...
{
  if (g_atomic_int_dec_and_test (&subscriber->ref_count))
    {
 ...
 * subscription ID from g_dbus_connection_signal_subscribe(), a strong or weak
 * reference to that #GObject must be passed to @user_data, and g_object_unref()
 * passed to @user_data_free_func. You are responsible for breaking the
 * resulting reference count cycle by explicitly unsubscribing from the signal
 * when dropping the last external reference to the #GObject.
 *
