		        ...
		        GError        **error)
{
  ...
  g_return_val_if_fail (G_IS_OUTPUT_STREAM (stream), FALSE);
  g_return_val_if_fail (vectors != NULL || n_vectors == 0, TRUE);
  g_return_val_if_fail (n_vectors >= 0, TRUE);
  ...
  if (class->writev_fn == NULL)
    {
      g_set_error_literal (error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED,
                           _("Output stream doesn’t implement writev"));
      return FALSE;
    }
			    ...
			    GError        **error)
{
  ...
  g_return_val_if_fail (G_IS_OUTPUT_STREAM (stream), FALSE);
  g_return_val_if_fail (vectors != NULL || n_vectors == 0, TRUE);
  g_return_val_if_fail (n_vectors >= 0, FALSE);
  ...
  while (n_vectors > 0)
    {
      ...
      while (n_written >= vectors[0].size)
        {
  ...
  if (result)
    {
      ...
      res = g_output_stream_writev_finish (G_OUTPUT_STREAM (stream), result, &nwritten, &error);
      data->bytes_written += nwritten;
      ...
      /* skip vectors that have been written in full */
      while (nwritten >= data->vectors[0].size)
        {
          nwritten -= data->vectors[0].size;
          ++data->vectors;
          --data->n_vectors;
        }
      /* skip partially written vector data */
      if (nwritten > 0)
        {
          data->vectors[0].size -= nwritten;
          data->vectors[0].buffer = ((guint8 *) data->vectors[0].buffer) + nwritten;
        }
    }
                                  ...
                                  gpointer             user_data)
{
  ...
  if (g_output_stream_async_write_is_via_threads (stream))
    ...
    writev_all_callback (G_OBJECT (stream), NULL, task);
}
