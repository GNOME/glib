 ...
 * are exceeded. For example, when writing to a local file, the aggregate buffer
 * size must not exceed %G_MAXSSIZE bytes.
 *
 * Virtual: writev_fn
 *
 * Returns: %TRUE on success, %FALSE if there was an error
  ...
  if (result)
    {
      ...
      /* skip vectors that have been written in full */
      while (data->n_vectors > 0 && nwritten >= data->vectors[0].size)
        {
      ...
      /* skip partially written vector data */
      if (nwritten > 0 && data->n_vectors > 0)
        {
