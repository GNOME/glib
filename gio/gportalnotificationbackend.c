...
/* This is the max size the xdg portal allows for icons */
                       ...
                       GError       **error)
{
  ...
  else if (G_IS_MEMORY_OUTPUT_STREAM (data))
    {
      GBytes *bytes = NULL;
      GIcon *icon = NULL;
                 ...
                 gpointer                          user_data)
{
  ...
  else if (g_notification_sound_get_silent (sound))
    {
      g_task_set_task_data (task,
                        ...
                        gpointer             user_data)
{
  ...
  if (version > 1 && (sound = g_notification_get_sound (notification)))
    {
      parser_data_hold (data);
