...
#ifdef G_OS_WIN32
static in_addr_t g_socket_w32_get_adapter_ipv4_addr (const gchar *name_or_ip)
{
  ...
  if (if_index == 0)
    {
      ...
      wchar_name_or_ip = (wchar_t *) g_try_malloc ((strlen(name_or_ip) + 1) * sizeof(wchar_t));
      if (wchar_name_or_ip)
        mbstowcs (wchar_name_or_ip, name_or_ip, strlen(name_or_ip) + 1);
  ...
  do
    {
      malloc_iterations++;
      addr_buf = (PIP_ADAPTER_ADDRESSES) g_try_realloc (bufsize);
      if (addr_buf)
      ...
      if (any_match)
        {
          ...
          int uni_count = 0;
          PIP_ADAPTER_UNICAST_ADDRESS uni_addr = eth_adapter->FirstUnicastAddress;
          while (uni_addr)
            {
              uni_count++;
