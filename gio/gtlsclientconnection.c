 ...
 * #GTlsClientConnection that has previously received a session ticket
 * from the server, provided that ticket has not previously been used
 * for session resumption, since session ticket reuse would be a privacy
 * weakness. Using this function causes the ticket to be copied without
