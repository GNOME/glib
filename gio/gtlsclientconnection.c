 ...
 * highest mutually-supported protocol version with the peer. Be aware
 * that SSL 3.0 is generally disabled by the #GTlsBackend, so the
 * lowest-supported protocol version is probably not SSL 3.0.
 *
 * Since 2.58, this will additionally cause an RFC 7507 fallback SCSV to
