...
g_keyfile_settings_backend_constructed (GObject *object)
{
  ...
  if (kfsb->filename == NULL)
    kfsb->filename = g_build_filename (g_get_user_config_dir (),
  ...
  kfsb->file = g_file_new_for_path (kfsb->filename);
                                         ...
                                         GParamSpec   *pspec)
{
  ...
  switch ((GKeyfileSettingsBackendProperty)prop_id)
    ...
    case PROP_FILENAME:
      /* Construct only. */
      g_assert (kfsb->filename == NULL);
      kfsb->filename = g_value_dup_string (value);
      break;
                                         ...
                                         GParamSpec *pspec)
{
  ...
  switch ((GKeyfileSettingsBackendProperty)prop_id)
    {
    case PROP_FILENAME:
      g_value_set_string (value, kfsb->filename);
      break;
