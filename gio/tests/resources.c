...
/* Test handling for empty GResource files. An empty file should be considered
 * a valid empty GResource. */
static void
test_resource_data_empty (void)
{
  ...
  /* Check the resource is empty. */
  root_bytes = g_resource_lookup_data (resource, "/",
                                       G_RESOURCE_LOOKUP_FLAGS_NONE,
                                       &local_error);
  g_assert_error (local_error, G_RESOURCE_ERROR, G_RESOURCE_ERROR_NOT_FOUND);
  g_assert_null (root_bytes);
