...
test_trash_not_supported (void)
{
  ...
  g_test_message ("File: %s (dev: %" G_GUINT64_FORMAT ")",g_file_peek_path (file), file_stat.st_dev);

  g_assert_cmpint (g_stat (g_get_home_dir (), &home_stat), ==, 0);
  g_test_message ("Home: %s (dev: %" G_GUINT64_FORMAT ")", g_get_home_dir (), home_stat.st_dev);
