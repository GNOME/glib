...
test_standalone_interface_info (void)
{
  ...
  for (i = 0; skel_info->methods[i] != NULL; i++)
    {
      g_assert_nonnull (slim_info->methods[i]);
      g_assert_cmpstr (skel_info->methods[i]->name, ==, slim_info->methods[i]->name);
      assert_arg_infos_equal (skel_info->methods[i]->in_args, slim_info->methods[i]->in_args);
      assert_arg_infos_equal (skel_info->methods[i]->out_args, slim_info->methods[i]->out_args);
    }
  g_assert_null (slim_info->methods[i]);

  for (i = 0; skel_info->signals[i] != NULL; i++)
    {
      g_assert_nonnull (slim_info->signals[i]);
      g_assert_cmpstr (skel_info->signals[i]->name, ==, slim_info->signals[i]->name);
      assert_arg_infos_equal (skel_info->signals[i]->args, slim_info->signals[i]->args);
    }
  g_assert_null (slim_info->signals[i]);

  for (i = 0; skel_info->properties[i] != NULL; i++)
    {
      g_assert_nonnull (slim_info->properties[i]);
      g_assert_cmpstr (skel_info->properties[i]->name, ==, slim_info->properties[i]->name);
      g_assert_cmpstr (skel_info->properties[i]->signature, ==, slim_info->properties[i]->signature);
      g_assert_cmpuint (skel_info->properties[i]->flags, ==, slim_info->properties[i]->flags);
    }
  g_assert_null (slim_info->properties[i]);

  /* FIXME: compare annotations on all nodes */
