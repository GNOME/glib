...
static gboolean
handle_no_annotation_with_fd_list (FooiGenFDPassing *object,
                                   GDBusMethodInvocation *invocation,
                                   GUnixFDList *fd_list,
...
#ifdef GLIB_MIN_VERSION_2_64
  /* If --glib-min-version >= 2.64 we emit GUnixFDList arguments when there's
   * an 'h' parameter, even without an explicit C.UnixFD annotation.
   */
  iface.handle_no_annotation = handle_no_annotation_with_fd_list;
#else
