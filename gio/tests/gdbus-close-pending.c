...
#else
# error This test is currently Unix-specific due to use of pipe()
#endif
           ...
           gconstpointer  context)
{
  ...
  pipe_res = pipe (f->server_to_client);
  g_assert_cmpint (pipe_res, ==, 0);
  pipe_res = pipe (f->client_to_server);
  g_assert_cmpint (pipe_res, ==, 0);
