...
test_schema_source (void)
{
  ...
  /* Test handling of empty compiled files. */
  source = g_settings_schema_source_new_from_directory ("schema-source-empty", NULL, TRUE, &error);
  g_assert_no_error (error);
  g_assert_nonnull (source);

  g_settings_schema_source_list_schemas (source, TRUE,
                                         &non_relocatable_schemas,
                                         &relocatable_schemas);
  g_assert_nonnull (non_relocatable_schemas);
  g_assert_cmpuint (g_strv_length (non_relocatable_schemas), ==, 0);
  g_assert_nonnull (relocatable_schemas);
  g_assert_cmpuint (g_strv_length (relocatable_schemas), ==, 0);
