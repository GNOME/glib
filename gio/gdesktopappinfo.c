  ...
  if (desktop_file_dirs == NULL || desktop_file_dirs->len == 0)
    {
      ...
      tmp = g_ptr_array_new_with_free_func ((GDestroyNotify) desktop_file_dir_unref);
      ...
      g_ptr_array_add (tmp, desktop_file_dir_ref (desktop_file_dir_user_config));

      /* Next, the system configs (/etc/xdg, and so on). */
      dirs = g_get_system_config_dirs ();
      for (i = 0; dirs[i]; i++)
        g_ptr_array_add (tmp, desktop_file_dir_new_for_config (dirs[i]));
      ...
      g_ptr_array_add (tmp, desktop_file_dir_ref (desktop_file_dir_user_data));

      /* Following that, XDG_DATA_DIRS/applications, in order */
      dirs = g_get_system_data_dirs ();
      for (i = 0; dirs[i]; i++)
        g_ptr_array_add (tmp, desktop_file_dir_new (dirs[i]));

      /* The list of directories will never change after this, unless
       * g_get_user_config_dir() changes due to %G_TEST_OPTION_ISOLATE_DIRS. */
      desktop_file_dirs = g_steal_pointer (&tmp);
