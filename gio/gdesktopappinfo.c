                                                     ...
                                                     gboolean     include_fallback)
{
  ...
  for (i = 0; types[i] != NULL; i++)
    g_message ("%s: %u, %s", G_STRFUNC, i, types[i]);
  // running with ISOLATE_DIRS limits this to @content_type itself (text/x-python)
  // running without also lists application/x-executable and text/plain
