...
desktop_file_dir_unindexed_read_mimeapps_lists (DesktopFileDir *dir)
{
  ...
  if (g_access (filename, R_OK) == 0)
    {
      g_warning ("The file %s is deprecated.  Please use gnome-mimeapps.list or mimeapps.list instead.", filename);
      desktop_file_dir_unindexed_read_mimeapps_list (dir, filename, "Added Associations", FALSE);
    }
  g_free (filename);
      ...
      for (i = 0; tweaks->additions[i]; i++)
        {
          ...
          if (!desktop_file_dir_app_name_is_masked (dir, app_name) &&
            ...
            {
              g_hash_table_insert (hits, app_name, GUINT_TO_POINTER (*n_hits));
              (*n_hits)++;
            }
        }
    }
      ...
      for (i = 0; tweaks->removals[i]; i++)
        {
          ...
          if (!desktop_file_dir_app_name_is_masked (dir, app_name) &&
              !g_hash_table_contains (blacklist, app_name) &&
              !g_hash_table_contains (hits, app_name))
            g_hash_table_add (blacklist, app_name);
        }
    }
}
 ...
 * @hits: the hashtable to store the hits
 * @n_hits: the number of hits in the table
 * @blacklist: the hashtable to store the blacklist
 *
 ...
 *
 * @n_hits is used as a sequence counter when adding items to @hits
 * (which is a table mapping desktop ids to integers) so that the order
 * of the hits can be determined.
 */
static void
desktop_file_dir_mime_lookup (DesktopFileDir *dir,
                              ...
                              GHashTable     *blacklist)
{
  desktop_file_dir_unindexed_mime_lookup (dir, mime_type, hits, n_hits, blacklist);
}
...
g_desktop_app_info_get_desktop_ids_for_content_type (const gchar *content_type,
                                                     gboolean     include_fallback)
{
  GHashTable *hits, *blacklist;
  guint n_hits;
  gchar **result;
  gchar **types;
  gint i, j;

  hits = g_hash_table_new (g_str_hash, g_str_equal);
  ...
  for (i = 0; types[i]; i++)
    for (j = 0; j < n_desktop_file_dirs; j++)
      desktop_file_dir_mime_lookup (&desktop_file_dirs[j], types[i], hits, &n_hits, blacklist);

  g_assert_cmpint (n_hits, ==, g_hash_table_size (hits));

  result = g_new (gchar *, n_hits + 1);

  {
    GHashTableIter iter;
    gpointer key, value;

    g_hash_table_iter_init (&iter, hits);
    while (g_hash_table_iter_next (&iter, &key, &value))
      result[GPOINTER_TO_UINT (value)] = g_strdup (key);
  }
  ...
  g_hash_table_unref (blacklist);
  g_hash_table_unref (hits);
  g_strfreev (types);
