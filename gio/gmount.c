 ...
 *
 * Note that on some system (Windows at least), the returned value may not be
 * the nicest display name, but calling g_mount_get_icon() will trigger the
 * icon request in a thread (as it can be very slow in some cases, such as
 * unresponsive or network mounts). If you care about displaying the most
 * adapted icon, you may want to initially use the returned icon, then connect
 * to the "changed" signal to later update code where it is used.
 *
