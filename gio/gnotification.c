 ...
 *
 * If a body was set via [func@Gio.Notification.set_markup_body] then @body is
 * only used for platforms that don't support markup.
 *
 * There is no need to set @body as a fallback when using
 * [func@Gio.Notification.set_markup_body] since markup will be stripped as fallback.
 *
 * Since: 2.40
 */
void
 ...
 * body for @notification, else the body set via [func@Gio.Notification.set_body]
 * is used. If no body is set via [func@Gio.Notification.set_body] @markup_body 
 ...
 * @notification: a `GNotification`
 * @sound: (nullable): a `GNotificationSound`
 *
  ...
  if (!g_action_parse_detailed_name (detailed_action, &action, &target, &error))
    {
      ...
      You can set body and markup body but markup body stripped for platforms that don't support markup
      g_error_free (error);
      return;
    }
