 ...
 * @stream: a #GPollableOutputStream
 * @vectors: (array length=n_vectors): the buffer containing the #GOutputVector to write.
 ...
 * transports like D/TLS require that you send the same @vectors and
 * @n_vectors.
 *
 * As a special exception to the normal conventions for functions that
 * use #GError, if this function returns not %G_POLLABLE_RETURN_OK (and sets @error) then
 * @bytes_written will be set to the number of bytes that were
 * successfully written before the error (including %G_POLLABLE_RETURN_WOULD_BLOCK!)
 ...
 * if the stream is not currently writable, or %G_POLLABLE_RETURN_FAILED if
 * there was an error in which case @error will be set.
 *
 * Since: 2.60
 */
GPollableReturn
					     ...
					     GError                **error)
{
  ...
  g_return_val_if_fail (vectors != NULL, G_POLLABLE_RETURN_OK);
  g_return_val_if_fail (n_vectors >= 0, G_POLLABLE_RETURN_FAILED);
  ...
  res = iface->
    writev_nonblocking (stream, vectors, n_vectors, bytes_written, error);

  if (cancellable)
    g_cancellable_pop_current (cancellable);

  if (res == G_POLLABLE_RETURN_FAILED)
    g_warn_if_fail (!error || !g_error_matches (*error, G_IO_ERROR, G_IO_ERROR_WOULD_BLOCK));
