                 ...
                 GMemoryMonitorDbus *dbus)
{
  ...
  level_s = g_variant_get_string (parameters, NULL);
  if (!level_s)
    return;

  if (g_strcmp0 (level_s, "low") == 0)
    level = G_MEMORY_MONITOR_WARNING_LEVEL_LOW;
  else if (g_strcmp0 (level_s, "medium") == 0)
    level = G_MEMORY_MONITOR_WARNING_LEVEL_MEDIUM;
  else if (g_strcmp0 (level_s, "critical") == 0)
    level = G_MEMORY_MONITOR_WARNING_LEVEL_CRITICAL;
  else
    {
      /* Consider all the unhandled levels to be "medium" */
      level = G_MEMORY_MONITOR_WARNING_LEVEL_MEDIUM;
    }
