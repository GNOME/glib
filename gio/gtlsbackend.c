 ...
 * @backend: the #GTlsBackend
 * @database: (allow-none): the #GTlsDatabase
 *
 ...
 */
void
g_tls_backend_set_default_database (GTlsBackend * backend, GTlsDatabase * database)
{
  ...
  G_LOCK (default_database_lock);
  if (default_database)
    g_object_unref (default_database);
  default_database = database ? g_object_ref (database) : NULL;
  G_UNLOCK (default_database_lock);
}
