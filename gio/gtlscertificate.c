...
g_tls_certificate_class_init (GTlsCertificateClass *class)
{
   ...
   *
   * A URI referencing the PKCS11 object containing a certificate.
   *
   * If %NULL the certificate is either not backed by PKCS11 or the
   * #GTlsBackend does not support PKCS11.
   *
   * Since: 2.62
   */
  g_object_class_install_property (gobject_class, PROP_PKCS11_URI,
				   g_param_spec_string ("pkcs11-uri",
							P_("PKCS11 URI"),
							P_("The PKCS11 URI for the certificate"),
							NULL,
							G_PARAM_READWRITE |
							G_PARAM_CONSTRUCT_ONLY |
							G_PARAM_STATIC_STRINGS));
 ...
 * @uri: A PKCS11 URI for the certificate
 * @error: #GError for error reporting, or %NULL to ignore.
 *
 * Creates a #GTlsCertificate from a PKCS11 URI. This function
  ...
  if (cert != NULL)
    {
      ...
      if (objects_uri == NULL)
        {
          g_set_error_literal (error, G_IO_ERROR, G_IO_ERROR_NOT_SUPPORTED, "This GTlsBackend does not support creating PKCS11 certificates");
          g_object_unref (cert);
          return NULL;
        }
      g_free (objects_uri);
