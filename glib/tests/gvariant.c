...
test_unaligned_construction (void)
{
  ...
  const struct {
    ...
    G_VARIANT_TYPE_UINT64, sizeof (guint64),
    G_VARIANT_TYPE_UINT32, sizeof (guint32),
    G_VARIANT_TYPE_UINT16, sizeof (guint16),
    G_VARIANT_TYPE_BYTE, sizeof (guint8),
  };
  ...
  for (i = 0; i < G_N_ELEMENTS (vectors); i++)
    {
      for (offset = 0; offset < vectors[i].size; offset++)
        {
