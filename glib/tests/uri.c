...
test_uri_parsing_relative (void)
{
  ...
  resolved = g_uri_resolve_relative (NULL, "http://a", 0, &error);
  g_assert_no_error (error);
  g_assert_cmpstr (resolved, ==, "http://a");
  g_free (resolved);

  resolved = g_uri_resolve_relative ("http://a", "b", 0, &error);
  g_assert_no_error (error);
  g_assert_cmpstr (resolved, ==, "http://a/b");
  g_free (resolved);

  resolved = g_uri_resolve_relative (NULL, "a", 0, &error);
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_MISC);
  g_clear_error (&error);

  resolved = g_uri_resolve_relative ("../b", "a", 0, &error);
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_MISC);
  g_clear_error (&error);

  resolved = g_uri_resolve_relative ("%%", "a", 0, &error);
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_MISC);
  g_clear_error (&error);
}
...
test_uri_to_string (void)
{
  ...
  uri = g_uri_build (0, "scheme", "userinfo", "host", 1234,
                     "/path", "query", "fragment");
  ...
  uri = g_uri_build_with_user (0, "scheme", "user", "pass", "auth", "host", 1234,
                               "/path", "query", "fragment");
  tostring = g_uri_to_string (uri);
...
test_uri_split (void)
{
...
#if 0
  TODO: normalizes several time.. (guri.c:707)
  g_uri_split_network ("scheme://user:pass%00;auth@host",
                       G_URI_FLAGS_HAS_PASSWORD|G_URI_FLAGS_HAS_AUTH_PARAMS,
                       NULL, NULL, NULL, &error);
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_PASSWORD);
  ...
  g_uri_split_network ("scheme://user:pass;auth%00@host",
                       G_URI_FLAGS_HAS_PASSWORD|G_URI_FLAGS_HAS_AUTH_PARAMS,
                       NULL, NULL, NULL, &error);
...
test_uri_is_valid (void)
{
  ...
  g_assert (g_uri_is_valid ("http://[::192.9.5.5]/ipng", 0, NULL));
  g_assert (g_uri_is_valid ("http://127.127.127.127/", 0, NULL));
  g_assert (g_uri_is_valid ("http://127.127.127.b/", 0, NULL));
  g_assert (g_uri_is_valid ("http://\xc3\x89XAMPLE.COM/", 0, NULL));

  g_assert (g_uri_is_valid ("  \r http\t://f oo  \t\n ", 0, NULL));
  g_assert (g_uri_is_valid ("  \r http\t://f oo  \t\n ", G_URI_FLAGS_PARSE_STRICT, NULL));

  g_assert (!g_uri_is_valid ("http://[::192.9.5.5/ipng", 0, &error));
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_HOST);
  g_clear_error (&error);

  g_assert (!g_uri_is_valid ("http://[fe80::dead:beef%wef%]/", 0, &error));
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_HOST);
  g_clear_error (&error);

  g_assert (!g_uri_is_valid ("http://%00/", G_URI_FLAGS_NON_DNS, &error));
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_HOST);
  g_clear_error (&error);

  g_assert (g_uri_is_valid ("http://foo/", G_URI_FLAGS_NON_DNS, &error));

  g_assert (!g_uri_is_valid ("http://%00/", 0, &error));
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_HOST);
  g_clear_error (&error);

  g_assert (!g_uri_is_valid ("http://%30.%30.%30.%30/", 0, &error));
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_HOST);
  g_clear_error (&error);

  g_assert (!g_uri_is_valid ("http://host:port", 0, &error));
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_PORT);
  g_clear_error (&error);

  g_assert (!g_uri_is_valid ("http://host:65536", 0, &error));
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_PORT);
  g_clear_error (&error);

  g_assert (!g_uri_is_valid ("http://host:6553l", 0, &error));
  g_assert_error (error, G_URI_ERROR, G_URI_ERROR_BAD_PORT);
  g_clear_error (&error);
}
...
test_uri_join (void)
{
  ...
  uri = g_uri_join_with_user (0, "scheme", "user\001", "pass\002", "authparams\003",
