...
static void
test_read_line_unicode_paragraph_separator (void)
{
  uint8_t *test_data = NULL;
  size_t test_data_len = 0;
  int fd;
  ...
  g_test_summary ("Test that reading a line when using a (partial) unicode paragraph separator doesn’t over-read the buffer.");
  ...
  test_data_len = 2047;
  test_data = g_malloc (test_data_len);
  /* use the beginning of a unicode line terminator, but don't add the rest */
  memset (test_data, '\xe2', test_data_len);
  g_file_set_contents (filename, (const gchar *) test_data, test_data_len, &local_error);
  g_assert_no_error (local_error);
      ...
      char *argv[])
{
  ...
  g_test_add_func ("/io-channel/read-line/unicode-paragraph-separator", test_read_line_unicode_paragraph_separator);
