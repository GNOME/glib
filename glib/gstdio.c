 ...
 */
static void
_g_win32_fill_privatestat (struct __stat64            *statbuf,
                           BY_HANDLE_FILE_INFORMATION *handle_info,
                              ...
                              gboolean    terminate)
{
  ...
  gboolean destructive_terminator = FALSE;

  if ((buf == NULL && alloc_buf == NULL && reparse_tag == NULL) ||
  ...
  dic_succeeded = DeviceIoControl (h, FSCTL_GET_REPARSE_POINT, NULL, 0,
                                   max_buffer,
                                   max_buffer_size,
                                   &returned_bytes, NULL);
  if (!dic_succeeded)
  ...
  if (rep_buf->ReparseTag == IO_REPARSE_TAG_SYMLINK)
    {
      BYTE *data = &((BYTE *) rep_buf->SymbolicLinkReparseBuffer.PathBuffer)[rep_buf->SymbolicLinkReparseBuffer.SubstituteNameOffset];
      ...
      if (buf)
        ...
        {
          *alloc_buf = g_malloc (to_copy + 2);
          memcpy (*alloc_buf, data, to_copy);
        }
    }
  else if (rep_buf->ReparseTag == IO_REPARSE_TAG_MOUNT_POINT)
    {
      BYTE *data = &((BYTE *) rep_buf->MountPointReparseBuffer.PathBuffer)[rep_buf->MountPointReparseBuffer.SubstituteNameOffset];
  ...
  if (destructive_terminator && to_copy > 1)
    buf[to_copy - 2] = 0;
  if (destructive_terminator && to_copy > 0)
    buf[to_copy - 1] = 0;
  if (to_copy > 0 && alloc_buf && ((char *) *alloc_buf)[to_copy] != '\0')
    ((char *) *alloc_buf)[to_copy] = ((char *) *alloc_buf)[to_copy + 1] = 0;
 ...
 * @buf: a buffer to receive the reparse point target path
                       ...
                       gboolean      terminate)
{
  ...
  if ((alloc_buf == NULL && buf == NULL) ||
      (alloc_buf != NULL && buf != NULL))
    {
      errno = EINVAL;
      return -1;
    }
