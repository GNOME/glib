...
#define SOURCE_DESTROYED(source) \
  ((g_atomic_int_get (&((source)->priv->flags)) & G_HOOK_FLAG_ACTIVE) == 0)
	      ...
	      guint         struct_size)
{
  ...
  g_atomic_int_set (&source->priv->flags, G_HOOK_FLAG_ACTIVE);
  ...
  if (!SOURCE_DESTROYED (source))
    {
      ...
      g_atomic_int_and (&source->priv->flags, ~G_HOOK_FLAG_ACTIVE);
			  ...
			  gboolean  can_recurse)
{
  ...
  if (can_recurse)
    g_atomic_int_or (&source->priv->flags, G_SOURCE_CAN_RECURSE);
  else
    g_atomic_int_and (&source->priv->flags, ~G_SOURCE_CAN_RECURSE);
...
g_source_get_can_recurse (GSource  *source)
{
  ...
  return (g_atomic_int_get (&source->priv->flags) & G_SOURCE_CAN_RECURSE) != 0;
}
...
block_source (GSource *source)
{
  ...
  g_atomic_int_or (&source->priv->flags, G_SOURCE_BLOCKED);
...
unblock_source (GSource *source)
{
  ...
  g_atomic_int_and (&source->priv->flags, ~G_SOURCE_BLOCKED);
...
g_main_dispatch (GMainContext *context)
{
  ...
  for (i = 0; i < context->pending_dispatches->len; i++)
      ...
      if (!SOURCE_DESTROYED (source))
	{
	  ...
	  if ((g_atomic_int_get (&source->priv->flags) & G_SOURCE_CAN_RECURSE) == 0)
	    block_source (source);
	  
          was_in_call = g_atomic_int_or (&source->priv->flags,
	  ...
	  if (!was_in_call)
            g_atomic_int_and (&source->priv->flags, ~G_HOOK_FLAG_IN_CALL);
