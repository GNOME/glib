...
g_get_monotonic_time (void)
{
  ...
  #define MACH_TIME_T __uint128_t
#else
  #define MACH_TIME_T guint64
#endif
...
g_get_monotonic_time (void)
{
  ...
  if (timebase_info.numer != timebase_info.denom)
    {
      // TODO: look at using __uint128_t where available
      MACH_TIME_T t_high, t_low;
      MACH_TIME_T result_high, result_low;

      /* 64 bit x 32 bit / 32 bit with 96-bit intermediate 
       * algorithm lifted from qemu */
      t_low = (val & 0xffffffffLL) * (MACH_TIME_T) timebase_info.numer;
      t_high = (val >> 32) * (MACH_TIME_T) timebase_info.numer;
      t_high += (t_low >> 32);
      result_high = t_high / (MACH_TIME_T) timebase_info.denom;
      result_low = (((t_high % (MACH_TIME_T) timebase_info.denom) << 32) +
                    (t_low & 0xffffffff)) /
                   (MACH_TIME_T) timebase_info.denom;
