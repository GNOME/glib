...
g_main_context_unref (GMainContext *context)
{
   ...
   * to setting the next/prev/context pointers in the sources to NULL above,
   * this won't ever access the context. We already removed the sources
   * completely from the context above. */
  for (s_iter = remaining_sources; s_iter; s_iter = s_iter->next)
    {
      source = s_iter->data;
