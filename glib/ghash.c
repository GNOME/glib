...
static inline gpointer
g_hash_table_fetch_key_or_value (gpointer a, guint index, gboolean is_big G_GNUC_UNUSED)
{
#ifdef USE_SMALL_ARRAYS
...
static inline void
g_hash_table_assign_key_or_value (gpointer a, guint index, gboolean is_big G_GNUC_UNUSED, gpointer v)
{
#ifdef USE_SMALL_ARRAYS
  if (is_big)
    *(((gpointer *) a) + index) = v;
...
static inline gpointer
g_hash_table_evict_key_or_value (gpointer a, guint index, gboolean is_big G_GNUC_UNUSED, gpointer v)
{
#ifdef USE_SMALL_ARRAYS
  if (is_big)
    {
...
get_status_bit (const guint32 *status_bits, guint index)
{
  return (status_bits [index / 32] >> (index % 32)) & 1;
}

static inline void
set_status_bit (guint32 *status_bits, guint index)
{
  status_bits [index / 32] |= 1 << (index % 32);
}
...
static void fname (GHashTable *hash_table, guint old_size, guint32 *status_bits) \
{                                                                       \
    guint i;                                                            \
                                                                        \
    for (i = 0; i < old_size; i++)                                      \
    {                                                                   \
      ...
      if (get_status_bit (status_bits, i))                              \
        continue;                                                       \
                                                                        \
      hash_table->hashes[i] = UNUSED_HASH_VALUE;                        \
      ...
      for (;;)                                                          \
        {                                                               \
          ...
          hash_val = g_hash_table_hash_to_index (hash_table, node_hash); \
                                                                        \
          while (get_status_bit (status_bits, hash_val))                \
            {                                                           \
              step++;                                                   \
              hash_val += step;                                         \
              hash_val &= hash_table->mask;                             \
                                                                        ...
                                                                        \
          set_status_bit (status_bits, hash_val);                       \
                                                                        \
  ...
  if (hash_table->size > old_size)
    {
      ...
      status_bits = g_new0 (guint32, (hash_table->size + 31) / 32);
    }
  else
    {
      status_bits = g_new0 (guint32, (old_size + 31) / 32);
    }

  if (is_a_set)
    resize_set (hash_table, old_size, status_bits);
  else
    resize_map (hash_table, old_size, status_bits);
