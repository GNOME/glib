 ...
 * The way to clean up the type must have been defined by using the macros
 * G_DEFINE_AUTO_CLEANUP_CLEAR_FUNC() and G_DEFINE_AUTO_CLEANUP_FREE_FUNC().
 *
 ...
 * The variable is cleaned up in a way appropriate to its type when the
 * variable goes out of scope.  The type must support this.
 * The way to clean up the type must have been defined by using the macro
