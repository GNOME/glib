...
static const gchar *
g_strnpbrk (const gchar *s, const gchar *separators, gsize n)
{
  gsize i;
                    ...
                    GUriParamsFlags  flags)
{
  ...
  while (attr < end)
    {
      value_end = g_strnpbrk (attr, separators, end - attr);
      if (!value_end)
        value_end = end;
      ...
      if (!uri_decode_www_form (&decoded_attr, attr, attr_end - attr,
      ...
      value = attr_end + 1;
      if (!uri_decode_www_form (&decoded_value, value, value_end - value,
                                www_form, G_URI_FLAGS_NONE, G_URI_ERROR_MISC, NULL))
        {
          g_free (decoded_attr);
          g_hash_table_destroy (hash);
