...
g_io_win32_check (GSource *source)
{
  ...
  switch (channel->type)
	...
	g_print (" MSG\n");
      ...
      g_critical ("Cannot check for win32 window messages using GIOChannel "
	...
	return TRUE;
      ...
      g_critical ("Cannot check for console messages using GIOChannel "
          "since glib was built for Universal Windows Platform apps");
#endif
      return FALSE;
		     ...
		     GError    **err)
{
  ...
  *bytes_read = sizeof (MSG);
  ...
  g_set_error_literal (err, G_IO_CHANNEL_ERROR, G_IO_CHANNEL_ERROR_FAILED,
                       "Cannot read win32 window messages since glib was built "
		      ...
		      GError     **err)
{
  ...
  *bytes_written = sizeof (MSG);
  ...
  g_set_error_literal (err, G_IO_CHANNEL_ERROR, G_IO_CHANNEL_ERROR_FAILED,
                       "Cannot write win32 window message since glib was built "
                       "for Universal Windows Platform apps");
  return G_IO_STATUS_ERROR;
#endif
}
...
#endif
{
  ...
  g_critical ("win32 window message I/O will not work since glib was built for "
  ...
  if (st->st_mode & _S_IFCHR) /* console */
    {
      ...
      g_critical ("console I/O will not work since glib was built for "
          "Universal Windows Platform apps");
#endif
				...
				GPollFD      *fd)
{
  ...
  switch (win32_channel->type)
    ...
    case G_IO_WIN32_CONSOLE:
      ...
      g_critical ("Cannot create a PollFD to handle console I/O since glib was "
          "built for Universal Windows Platform apps");
      fd->fd = -1;
#endif
      break;
    ...
    case G_IO_WIN32_WINDOWS_MESSAGES:
      ...
      g_critical ("Cannot create a PollFD to handle win32 window messages "
          "since glib was built for Universal Windows Platform apps");
      fd->fd = -1;
#endif
      break;
